...
void Player::RecalculateStats() {
  m_attributes[MAXHITPOINTS] =
      10 + (static_cast<int>(m_level * (GetAttr(HEALTH) / 1.5)));
...
int Player::GetItemIndex(const std::string &p_name) {
  item *i = double_find_if(m_inventory, m_inventory + MaxItems(),
...
void Player::SendString(const std::string &p_string) {
  ...
  if (Conn() == 0) {
    ERRORLOG.Log("Trying to send string to player " + Name() +
                 " but player is not connected.");
    return;
  }
...
ostream &operator<<(ostream &p_stream, const Player &p) {
  ...
  insert(p_stream, p.m_nextattacktime);
  p_stream << "\n";
...
istream &operator>>(istream &p_stream, Player &p) {
  ...
  p_stream >> temp;
  extract(p_stream, p.m_nextattacktime);
  p_stream >> p.m_baseattributes;
